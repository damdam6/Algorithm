...
//			}
			...
			for (int i = 0; i < 4; i++) {
				...
				if (arr[nx][ny].wall == 1 && !tmp.destroy && tmp.stage + 1 < vtArr[nx][ny].stageTrue) {
					vtArr[nx][ny].stageTrue = tmp.stage + 1;
					qu.add(new block(nx, ny, tmp, true));
				}
				if (arr[nx][ny].wall == 1)continue;
				
				if(tmp.destroy && tmp.stage + 1 < vtArr[nx][ny].stageTrue ) {
					vtArr[nx][ny].stageTrue = tmp.stage +1;
					qu.add(new block(nx, ny, tmp, false));
				}else if(!tmp.destroy && tmp.stage + 1 < vtArr[nx][ny].stageFalse){
					vtArr[nx][ny].stageFalse = tmp.stage +1;
					qu.add(new block(nx, ny, tmp, false));
				}
		...
		int min = Math.min(vtArr[N-1][M-1].stageFalse, vtArr[N-1][M-1].stageTrue);
